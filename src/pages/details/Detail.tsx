...
const Detail = () => {
  ...
  const fetchData = async () => {
    await instance
      ...
      .then(( res : any) => setScore(res.data.list));
  };
  ...
  const scoreAddHandler = async ( ) => {
    setScoreAdd(true);
     await instance.put('/individualscore').then((res)=>{
      return alert(res.data.status) 
  ...
  console.log(scoreAddHandler, '삭제');
  useEffect(() => {
            ...
            <div>
              ...
              <div>
                {score.map((scoreList: any, i: any) => (
                  ...
                  >
                    <img className='w-11 h-11' src={cycling} alt='운동그림' />
